...
import Automata.Layout.Constrained qualified as Constrained
...
layoutPositioned a = let (a', _, _) = untilStableOrCount 50 runStep (a, initialTemp, mkStdGen 8607857038680846995) in a'
 where
  ...
  untilStableOrCount n f x@(AL grps trs, _, _) = bool (untilStableOrCount (n - 1) f (f x)) x stable
   where
    stable = let (AL fgrps ftrs, _, _) = f x in abs (totalEnergy grps trs - totalEnergy fgrps ftrs) < 0.0001
...
runStep (a, tmp, gen) =
 ...
 where
  (_, _, AL sts2 trs2, gen2) = step (tmp, iterPerStep, a, gen)
...
nodeDistances groups = sum $ map energy $ concat groups
 where
  energy u = sum $ map (\v -> 1 / dist u v ** 2) $ concatMap (filter (\v -> psid v > psid u)) groups
...
edgeLengths groups transitions = sum $ map energy transitions
 where
  ...
  energy (T _ u v _ _) = dist (getPState u) (getPState v) ** 2

nodeEdgeDistances :: [[PositionedState]] -> [Transition t] -> Double
nodeEdgeDistances groups transitions = sum $ map energy $ concat groups
 where
  getPState sid = head $ concatMap (filter ((==) sid . psid)) groups
  energy s@PS{psid, x = sx, y = sy} = sum $ map distCost $ filter (\(T _ uId vId _ _) -> uId /= psid && vId /= psid) transitions
   where
    distCost (T _ u v _ _) = 1 / max 0.001 distance ** 2
     where
      ...
      distance =
        if betweenAnyOrder ux vx (sx + a + lambda * b) && betweenAnyOrder uy vy (sy + c + lambda * d)
